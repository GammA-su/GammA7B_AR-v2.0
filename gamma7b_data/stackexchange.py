import json
import re
import tempfile
import xml.etree.ElementTree as ET
from pathlib import Path
from typing import Dict, Optional, Tuple

from .schema import NormalizedDocument
from .utils import open_zst_writer, stable_hash


def strip_html(text: str) -> str:
    text = re.sub(r"<pre><code>(.*?)</code></pre>", r"\1", text, flags=re.DOTALL)
    text = re.sub(r"<code>(.*?)</code>", r"\1", text, flags=re.DOTALL)
    text = re.sub(r"<[^>]+>", " ", text)
    text = re.sub(r"\s+", " ", text)
    return text.strip()


def iter_posts(posts_path: Path):
    try:
        context = ET.iterparse(posts_path, events=("end",))
        for _, elem in context:
            if elem.tag.endswith("row"):
                yield elem.attrib
                elem.clear()
        return
    except ET.ParseError:
        pass

    text = posts_path.read_text(encoding="utf-8", errors="ignore")
    pos = 0
    while True:
        start = text.find("<row", pos)
        if start == -1:
            break
        i = start + 4
        in_quote = None
        while i < len(text):
            ch = text[i]
            if ch in ("\"", "'"):
                if in_quote is None:
                    in_quote = ch
                elif in_quote == ch:
                    in_quote = None
            elif ch == ">" and in_quote is None:
                tag_body = text[start + 4 : i]
                attrs = dict(re.findall(r'(\w+)="([^"]*)"', tag_body))
                if attrs:
                    yield attrs
                pos = i + 1
                break
            i += 1
        else:
            break


def resolve_posts_path(path: Path) -> Tuple[Path, Optional[tempfile.TemporaryDirectory]]:
    if path.is_dir():
        candidate = path / "Posts.xml"
        if not candidate.exists():
            raise FileNotFoundError(f"Posts.xml not found in {path}")
        return candidate, None
    if path.suffix == ".7z":
        try:
            import py7zr
        except ImportError as exc:
            raise ImportError("py7zr is required to extract .7z StackExchange archives.") from exc
        tmpdir = tempfile.TemporaryDirectory()
        with py7zr.SevenZipFile(path, mode="r") as archive:
            archive.extractall(path=tmpdir.name)
        candidate = Path(tmpdir.name) / "Posts.xml"
        if not candidate.exists():
            found = list(Path(tmpdir.name).rglob("Posts.xml"))
            if not found:
                raise FileNotFoundError("Posts.xml not found in archive")
            candidate = found[0]
        return candidate, tmpdir
    if path.suffix == ".xml":
        return path, None
    raise ValueError(f"Unsupported StackExchange input: {path}")


def ingest_stackexchange(
    input_path: Path,
    out_path: Path,
    limit: Optional[int] = None,
    domain: str = "forums_qa",
    source: str = "stackexchange",
    logger=None,
    log_every: int = 300,
) -> None:
    posts_path, tmpdir = resolve_posts_path(input_path)
    if logger:
        logger.info("StackExchange ingest config: posts=%s limit=%s", posts_path, limit)

    questions: Dict[str, dict] = {}
    answers_by_id: Dict[str, dict] = {}
    best_answers: Dict[str, dict] = {}

    for row in iter_posts(posts_path):
        post_type = row.get("PostTypeId")
        if post_type == "1":
            qid = row.get("Id")
            if not qid:
                continue
            questions[qid] = {
                "id": qid,
                "title": row.get("Title", ""),
                "body": strip_html(row.get("Body", "")),
                "tags": row.get("Tags", ""),
                "creation_date": row.get("CreationDate", ""),
                "accepted_answer_id": row.get("AcceptedAnswerId"),
                "score": int(row.get("Score", "0")),
            }
        elif post_type == "2":
            parent = row.get("ParentId")
            if not parent:
                continue
            answer = {
                "id": row.get("Id"),
                "body": strip_html(row.get("Body", "")),
                "score": int(row.get("Score", "0")),
            }
            answers_by_id[answer["id"]] = answer
            current = best_answers.get(parent)
            if current is None or answer["score"] > current["score"]:
                best_answers[parent] = answer
        if limit is not None and len(questions) >= limit:
            break
        if logger and len(questions) and len(questions) % log_every == 0:
            logger.info("StackExchange parse progress: questions=%s", len(questions))

    with open_zst_writer(out_path) as writer:
        count = 0
        for qid, question in questions.items():
            accepted_id = question.get("accepted_answer_id")
            answer = None
            if accepted_id and accepted_id in answers_by_id:
                answer = answers_by_id[accepted_id]
            if answer is None:
                answer = best_answers.get(qid, {"body": ""})

            text = f"Q: {question.get('title','')}\n{question.get('body','')}\nA: {answer.get('body','')}".strip()
            doc_id = question.get("id") or stable_hash(f"{source}:{text}")
            doc = NormalizedDocument(
                text=text,
                source=source,
                domain=domain,
                doc_id=str(doc_id),
                license_tag=None,
                created_at=question.get("creation_date"),
                meta={"tags": question.get("tags", "")},
            )
            writer.write((json.dumps(doc.to_json(), ensure_ascii=False) + "\n").encode("utf-8"))
            count += 1
            if logger and count % log_every == 0:
                logger.info("StackExchange ingest progress: docs=%s", count)
            if limit is not None and count >= limit:
                break
    if logger:
        logger.info("StackExchange ingest done: docs=%s out=%s", count, out_path)

    if tmpdir is not None:
        tmpdir.cleanup()
